                ...
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            {!! Form::open([
                                'route' => ['exchange.store', @$lims_sale_data->id ?? null],
                                'method' => 'Post',
                                'files' => true,
                                'id' => 'payment-form',
                            ]) !!}
    ...
    <script type="text/javascript">
        ...
        $("#myTable").on('focusout', '.qty', function() {
            ...
            if (is_variant[rowindex]) {
                checkQuantity(value, true, 'table.order-list');
            } else {
                checkDiscount(value, 'input', 'table.order-list');
            }
        });

        // ✅ Delete product - works for any table with .ibtnDel button
        ...
        // Edit product - both tables support
        ...
        $(document).on('click', 'table .minus', function() {
            ...
            if (qty > 0) {
                ...
                if (is_variant[rowindex])
                    ...
                    checkDiscount(qty, '3', tableSelector);
            } else {
                qty = 1;
            }
        });
        ...
        function addNewProduct(data, tableSelector) {
            ...
            if (tableSelector === 'table.return-order-list') {
                cols += '<td class="is-exchange">';
                cols += '  <input type="checkbox" id="exchange_' + data[1] +
                    '" name="is_exchange[]" class="exchange-checkbox" checked value="' + data[1] +
                    '" onchange="calculateExchangeValue()" style="width: 18px; height: 18px; cursor: pointer;">';
                cols += '</td>';
            } else {
                cols +=
        ...
        function edit() {
            ...
            // ✅ Detect which table the row belongs to
            var tableSelector = 'table.order-list';
            var $row = $('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')');
            if ($row.length === 0) {
                $row = $('table.return-order-list tbody tr:nth-child(' + (rowindex + 1) + ')');
            ...
            // ✅ Get product code and type from correct table
        ...
        function checkDiscount(qty, flag, tableSelector = 'table.order-list', price = 0) {
            ...
            $(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ') .qty').val(qty);
            flag = true;
            checkQuantity(String(qty), flag, tableSelector); // ✅ tableSelector pass করা
        }
            ...
            if (without_stock == 'no') {
                if (product_type && (product_type.trim() == 'standard' || product_type.trim() == 'combo')) {
                    ...
                    if (total_qty > qty) {
                        ...
                        if (sale_qty == 0) {
                            $(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ')').remove();
                        }
                    }
        ...
        function calculateRowProductData(quantity, tableSelector = 'table.order-list') {
            ...
            // ✅ topping price calculation - tableSelector দিয়ে
            var topping_price = ($(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ')').find('.topping-price')
                .val() * quantity) || 0;

            // ✅ সব value update - tableSelector দিয়ে
        ...
        $('#saleSearchBtn').on('click', function() {
            let reference = $('#sale_product_search').val();
