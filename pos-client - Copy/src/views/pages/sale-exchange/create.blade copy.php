                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    ...
                                                    <div class="table-responsive mt-3">
                                                        <table id="sale-product-table" class="table table-hover">
                                                            <thead>
                                                                <tr>
                                                                    <th>{{ __('db.name') }}</th>
                                                                    <th>{{ __('db.Code') }}</th>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        function calculateExchangeValue() {
            ...
            // Get values from the ORIGINAL products table (with exchange checkboxes)
            $('#pos-layout').find('input.exchange-checkbox:checked').each(function() {
                var row = $(this).closest('tr');
                var subtotal = parseFloat(row.find('.sub-total').text()) || 0;
                console.log('Checked product subtotal:', subtotal);
                exchangeValue += subtotal;
            });

            console.log('Total Exchange Value: ' + exchangeValue.toFixed(decimal));

            // Update exchange value display
            $('#exchange-value').text(exchangeValue.toFixed(decimal));
        ...
        function calculateNewProductsTotal() {
            ...
            // Get values from NEW products table (order-list)
            $('table.order-list tbody tr').each(function() {
                var subtotal = parseFloat($(this).find('.sub-total').text()) || 0;
                console.log('New product subtotal:', subtotal);
                newProductsTotal += subtotal;
            });

            console.log('Total New Products: ' + newProductsTotal.toFixed(decimal));

            // Update new products total display
            $('#new-products-total').text(newProductsTotal.toFixed(decimal));
        ...
        function calculateExchangeBalance() {
            ...
            console.log('Shipping Cost: ' + shippingCost.toFixed(decimal));
            console.log('Grand Total: ' + grandTotal.toFixed(decimal));
            console.log('Balance (Due): ' + balance.toFixed(decimal));
            console.log('============================');
        ...
        // Exchange checkbox change - Recalculate when exchange items are selected/deselected
        $(document).on('change', 'input.exchange-checkbox', function() {
            console.log('Exchange checkbox changed');
            calculateExchangeValue();
        });

        // Shipping cost change - Recalculate balance
        $('input[name="shipping_cost"]').on('blur change', function() {
            console.log('Shipping cost changed');
            calculateExchangeBalance();
        });

        // Product quantity change in new products table
        $("#myTable").on('focusout', '.qty', function() {
            console.log('New product quantity changed');
            setTimeout(function() {
                calculateNewProductsTotal();
            }, 100);
        });

        // Product deletion from new products table
        $("table.order-list tbody").on("click", ".ibtnDel", function(event) {
            setTimeout(function() {
                console.log('Product deleted from new products');
                calculateNewProductsTotal();
            }, 100);
        });

        // When new product is added to order-list table
        $(document).on('click', '.product-img', function() {
            setTimeout(function() {
        ...
        // Listen to calculateTotal() function calls to also update exchange calculations
        ...
        // Listen to calculateRowProductData() to update when row prices change
        var originalCalculateRowProductData = window.calculateRowProductData;
        window.calculateRowProductData = function(quantity) {
            if (originalCalculateRowProductData) {
                originalCalculateRowProductData.apply(this, arguments);
            }
            setTimeout(function() {
                calculateNewProductsTotal();
            }, 50);
