...
const BillerList = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const url = search ? `billers/list?search=${encodeURIComponent(search)}` : 'billers/list';
      ...
      msg.error(err?.response?.data?.message || 'Failed to load billers');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const openEdit = async (row) => {
    try {
      const res = await api.get('billers/edit/' + row.id);
      const data = res?.data;
      ...
      msg.error(err?.response?.data?.message || 'Failed to load biller');
    }
  };
  ...
  const handleSubmitAdd = async (e) => {
    ...
    try {
      ...
      const res = await api.post('billers/store').values(payload);
      const data = res?.data;
      if (res?.status === 200 && data?.status === 200) {
        msg.success(data?.message || 'Biller created');
        hideAddModal();
        fetchData();
      } else {
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      ...
      const res = await api.put('billers', selectedId).values(payload);
      const data = res?.data;
      if (res?.status === 200 && data?.status === 200) {
        msg.success(data?.message || 'Biller updated');
        hideEditModal();
        fetchData();
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await api.delete('billers/' + itemToDelete.id);
      const data = res?.data;
      if (res?.status === 200 && data?.status === 200) {
        setEntities((prev) => prev.filter((e) => e.id !== itemToDelete.id));
        msg.success(data?.message || 'Biller deleted');
      } else {
    ...
    <div>
      ...
      <div className="container-fluid">
        <button type="button" className="btn btn-info mr-2" onClick={openAdd}>
          <SafeFontAwesomeIcon icon={faPlus} className="mr-2" size="sm" /> Add Biller
        </button>
      ...
      <FormModal moduleName="Add Biller" modalState={showAddModal} toggleFormModal={hideAddModal} width="640px">
      ...
      <FormModal moduleName="Update Biller" modalState={showEditModal} toggleFormModal={hideEditModal} width="640px">
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>Are you sure you want to delete biller <strong>"{itemToDelete?.name}"</strong>?</p>
              </div>
              <div className="modal-footer">
