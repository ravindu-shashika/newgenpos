...
const Currency = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setEntities(response.data.currencies);
      setIsLoading(false);
    } catch (error) {
  ...
  const searchAndFetch = async (searchPhrase, selectedColumn) => {
    try {
      ...
      const response = await api.get(
        `currencies/search/${searchPhrase}/${selectedColumn}`,
      );
  ...
  const save = async () => {
    if (isEdit === false) {
      try {
        const response = await api.post('currencies').values(newData);
      ...
      try {
        const response = await api
          .update(`currencies/${selectedId}/update`)
          .values(newData);
