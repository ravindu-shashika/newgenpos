...
function Field({ label, required, children, tooltip }) {
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <label style={{ fontSize: 12, fontWeight: 600, letterSpacing: ".06em", textTransform: "uppercase", color: "var(--muted)" }}>
...
function NumInput({ addonIcon, ...props }) {
    const [focused, setFocused] = useState(false);
    const style = {
        ...inputStyle,
        borderColor: focused ? "#3b82f6" : "var(--border)",
...
function SectionLabel({ children }) {
    return (
        <div style={{
            ...
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: "var(--muted)", whiteSpace: "nowrap" }}>
                {children}
            </span>
...
export default function AddBarcodeSettings() {
    ...
    const handleSubmit = (e) => {
        ...
        setTimeout(() => {
            setSubmitted(false);
            setForm(INITIAL);
            showToast("Barcode sticker setting saved successfully!");
        }, 1200);
    };
    ...
    .bc-wrap * { box-sizing: border-box; margin: 0; padding: 0; }
    .bc-wrap {
      background: var(--bg); color: var(--ink);
      font-family: 'IBM Plex Mono', 'Fira Code', monospace;
      min-height: 100vh; padding: 32px 28px 64px;
    }
    .bc-wrap button { font-family: inherit; }
    .bc-wrap input[type=number]::-webkit-inner-spin-button,
    .bc-wrap input[type=number]::-webkit-outer-spin-button { opacity: .5; }
    .bc-card {
    ...
    .submit-btn {
      ...
      letter-spacing: .04em; padding: 12px 48px; transition: opacity .15s, transform .12s;
    }
            ...
            <div className="bc-wrap">
                <div className="bc-card">
                    <div className="bc-card-head">
                        <div className="bc-card-title">Add Barcode Sticker Setting</div>
                    </div>
                        ...
                        <form onSubmit={handleSubmit} noValidate>
                            ...
                            <div className="grid-1" style={{ marginBottom: 20 }}>
                                <Field label="Sticker Sheet Setting Name" required>
                                    <TextInput
                                        name="name" value={form.name} placeholder="Sticker Sheet Setting Name"
                                ...
                                <Field label="Sticker Sheet Setting Description">
                                    <TextArea
                                        ...
                                        placeholder="Sticker Sheet Setting Description" rows={3}
                            ...
                            {form.is_continuous && (
                                <div className="continuous-note">
                                    ...
                                    Continuous feed mode — paper height and stickers per sheet are not required.
                                </div>
                            )}
                            ...
                            <div className="grid-2" style={{ marginBottom: 24 }}>
                                <Field label="Additional Top Margin" required>
                                    <NumInput name="top_margin" value={form.top_margin} min={0} step={0.00001}
                                        addonIcon={<ArrowUpIcon />}
                                        placeholder="0"
                                        onChange={e => set("top_margin", e.target.value)} required />
                                </Field>
                                <Field label="Additional Left Margin" required>
                                    <NumInput name="left_margin" value={form.left_margin} min={0} step={0.00001}
                                        addonIcon={<ArrowLeftIcon />}
                                        placeholder="0"
                                        onChange={e => set("left_margin", e.target.value)} required />
                                </Field>
                            </div>
                            ...
                            <div className="grid-2" style={{ marginBottom: 24 }}>
                                <Field label="Width of Sticker" required>
                                    <NumInput name="width" value={form.width} min={0.1} step={0.00001}
                                        placeholder="e.g. 2.5"
                                        onChange={e => set("width", e.target.value)} required />
                                    {errors.width && <div className="err">{errors.width}</div>}
                                </Field>
                                <Field label="Height of Sticker" required>
                                    <NumInput name="height" value={form.height} min={0.1} step={0.00001}
                                        placeholder="e.g. 1.0"
                            ...
                            <div className="grid-2" style={{ marginBottom: 24 }}>
                                <Field label="Paper Width" required>
                                    <NumInput name="paper_width" value={form.paper_width} min={0.1} step={0.00001}
                                        placeholder="e.g. 8.5"
                                        onChange={e => set("paper_width", e.target.value)} required />
                                    {errors.paper_width && <div className="err">{errors.paper_width}</div>}
                                </Field>
                                {!form.is_continuous && (
                                    <Field label="Paper Height" required>
                                        <NumInput name="paper_height" value={form.paper_height} min={0.1} step={0.00001}
                                            placeholder="e.g. 11.0"
                                            onChange={e => set("paper_height", e.target.value)} required />
                            ...
                            <div className="grid-2" style={{ marginBottom: 24 }}>
                                <Field label="Stickers in One Row" required>
                                    <NumInput name="stickers_in_one_row" value={form.stickers_in_one_row} min={1} step={1}
                                        placeholder="e.g. 3"
                                        onChange={e => set("stickers_in_one_row", e.target.value)} required />
                                    {errors.stickers_in_one_row && <div className="err">{errors.stickers_in_one_row}</div>}
                                </Field>
                                {!form.is_continuous && (
                                    <Field label="No. of Stickers per Sheet" required>
                                        <NumInput name="stickers_in_one_sheet" value={form.stickers_in_one_sheet} min={1} step={1}
                                            placeholder="e.g. 30"
                                            onChange={e => set("stickers_in_one_sheet", e.target.value)} required />
                            ...
                            <div className="grid-2" style={{ marginBottom: 24 }}>
                                <Field label="Distance Between Two Rows" required>
                                    <NumInput name="row_distance" value={form.row_distance} min={0} step={0.00001}
                                        placeholder="0"
                                        onChange={e => set("row_distance", e.target.value)} required />
                                </Field>
                                <Field label="Distance Between Two Columns" required>
                                    <NumInput name="col_distance" value={form.col_distance} min={0} step={0.00001}
                                        placeholder="0"
                                        onChange={e => set("col_distance", e.target.value)} required />
                                </Field>
                            </div>
