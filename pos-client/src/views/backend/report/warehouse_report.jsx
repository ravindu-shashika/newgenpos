...
const TABS = [
    { id: 'sale', label: 'Sale', endpoint: 'report/warehouse-sale-data' },
    { id: 'purchase', label: 'Purchase', endpoint: 'report/warehouse-purchase-data' },
    { id: 'quotation', label: 'Quotation', endpoint: 'report/warehouse-quotation-data' },
    { id: 'return', label: 'Return', endpoint: 'report/warehouse-return-data' },
    { id: 'expense', label: 'Expense', endpoint: 'report/warehouse-expense-data' },
];

function canViewWarehouseReport() {
    if (permissionsBypassed()) return true;
    return authStore.getPermissions()?.includes('warehouse-report') ?? false;
}
...
function TabTable({ tab, filters, decimal }) {
    ...
    useEffect(() => { setPage(1); }, [filters.start_date, filters.end_date, filters.warehouse_id]);
...
export default function BackendReportWarehouseReport() {
    const { toast, showToast } = useToast();
    const [canView, setCanView] = useState(canViewWarehouseReport);
    ...
    useEffect(() => authStore.subscribe(() => setCanView(canViewWarehouseReport())), []);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setFormLoading(true);
            try {
                const res = await api.get('report/warehouse_report');
                const data = res.data ?? {};
                ...
                showToast(err?.message || 'Failed to load warehouse report.', 'error');
            } finally {
    ...
    const filters = useMemo(() => ({
        ready: !formLoading && startDate && endDate && warehouseId,
        start_date: startDate,
        end_date: endDate,
        warehouse_id: warehouseId,
    }), [formLoading, startDate, endDate, warehouseId]);

    if (!canView) {
        return (
            <PageLayout title="Warehouse Report">
