...
import InvoicePrintA4Half from '../../../printouts/InvoicePrintA4Half';
...
const Invoice = () => {
  ...
  const save = async () => {
    try {
      const response = await api.post('sales-invoices').values({
        invoice: newData,
        payment: paymentData,
  ...
  const checkBeforeSave = () => {
    ...
    if (parseFloat(paymentData.credit_amount) < 0) {
      msg.warning('Credit Amount minus, Please check the payments.');
      return false;
    }

    if (paymentData.note === '') {
      msg.warning('Payment Note Cannot Be Empty.');
      return false;
    }
    ...
    const tot_pay =
      ...
      parseFloat(paymentData.fc_amount) +
      parseFloat(paymentData.bank_deposit) +
      parseFloat(paymentData.return_amount) +
      parseFloat(paymentData.credit_amount);
    if (parseFloat(newData.tot_amount) != tot_pay) {
      msg.warning(
        'Invoice Total must be equals to total payments. Check the payments before save.',
      );
      return false;
    }
    ...
    if (e.key === 'Enter') {
      ...
      if (tag_item) {
        ...
        setItemRow({
          ...
          item: {
            itemcode: tag_item.item.itemcode,
            itemname: tag_item.item.itemname,
          },
          stone_details: tag_item.stone_details,
        });

        const st_details = tag_item.stone_details.map((st_item) => {
          return { ...st_item, index: uuidv4() };
        });

        setItemStoneRow(st_details);
  ...
  const printReceipt = async (invoice_no, is_dupplicate) => {
    const result = await InvoicePrintA4Half.loadInvoice(
      invoice_no,
          ...
          <div className="row mb-3">
            ...
            <div className="col-sm-3 border-left border-white ">
              ...
              <div className="form-group row mb-0">
                <label htmlFor="gold_rate" className="col-sm-4 col-form-label ">
                  Advance
                </label>
                <div className="col-sm-8">
                  <input
