...
const GoldMelt = () => {
  ...
  const [itemMetalRow , setItemMetalRow] = useState({
    metal_type: '',
    metal: {
      id: '',
      description: '',
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await api.get(`gold-melt`);
  ...
  const LoadFor = async () => {
    try {
      ...
      const response = await api.get(
        `forfeit-purchases-for-melt/${newData.purchase_no}/${newData.bc_no}`,
      );

        
        if(response.data.result){
          if(response.data.melted){
            msg.warning('This purchase already melted!');
            fetchData();
          }else{
            ...
            dataObj.purchase_no = dataObj.nno;
            dataObj.nno = newData.nno;
            dataObj.daily_gold_rate = parseFloat(response.data.daily_gold_rate.rate).toFixed(2);
            ...
            dataObj.metal_details = [];
            dataObj.stone_details = st_details;
            console.log(dataObj);
            setNewData(dataObj);
          }
        }else{
            msg.warning('This is not a melt purchase!');
            fetchData();
        }
  ...
  const handleItemRowChanges = (e) => {
    ...
    if (inputName === 'la_cost' && inputValue != '') {
      document.getElementById('gold_rate').readOnly = true;
    } else {
      document.getElementById('gold_rate').readOnly = false;
    }

    if (inputName === 'gold_rate' && inputValue != '') {
      document.getElementById('la_cost').readOnly = true;
      document.getElementById('wastage_per_pound').readOnly = true;
    } else {
      document.getElementById('la_cost').readOnly = false;
      document.getElementById('wastage_per_pound').readOnly = false;
    }
  ...
  const handleItemMetalRowChanges = (e) => {
    const targetInput = e.target;
    const inputName = targetInput.name;
    // const inputValue = targetInput.value;
    const inputValue =
  ...
  const save = async () => {
    try {
      if (!isEdit) {
        const response = await api.post('gold-melt').values(newData);
        ...
        //   .values(newData);
        const response = await api.put('gold-melt', newData.id).values(newData);
  ...
  const addNewMetalItem = (e) => {
   
    // if (checkBeforeAddItem() === false) return;
    setNewData({
      ...newData,
      metal_details: [...newData.metal_details, itemMetalRow],
    });
  ...
  const calColumnTotals = () => {
    ...
    newData.metal_details.forEach((row) => {
      // prettier-ignore-end
      // prettier-ignore
    ...
    setNewData({
      ...newData,
      melt_total_weight: total_weight,
    });
  };
  ...
  const printReceipt = (trans_no, is_dupplicate) => {
    ForfeitPurchasePrintA4Half.load(trans_no, cookie.get('user_branch'), is_dupplicate);
  };
        ...
        <div>
          <div className="row">
            ...
            <div className="col-sm-4 border-left border-white">
              ...
              <div className="form-group row mb-0">
                <label htmlFor="purchase_no" className="col-sm-5 col-form-label">
                  PO No
                </label>
                <div className="col-sm-7">
                  <input
              ...
              <table className="table table-bordered table-sm table-hover">
                <thead className="thead-dark text-center">
                  ...
                  {/* rowSpan="2" */}
                    ...
                    <td width="9%">Gold Type</td>
                    <td width="6%">Quantity</td>
                    <td width="9%">Weight (g)</td>
                    <td width="9%">Rate</td>
                    <td width="9%">LC</td>
                    <td width="9%">Wastage (8g)</td>
                    <td width="9%">Total Wastage</td>
                  ...
                  <tr>
                    ...
                    <td>
                      <input
                        type="number"
                        name="wastage_per_pound"
                        id="wastage_per_pound"
                        step="0.01"
                        className="form-control form-control-sm text-right"
                        value={itemRow.wastage_per_pound}
                        onChange={handleItemRowChanges}
                        onKeyDown={(event) => {
                          if (event.key === 'Enter') {
                            addCostForWastage();
                      ...
                      <tr key={item.index}>
                        ...
                        <td>
                          <input
                            ...
                            data-id={index}
                            className="form-control form-control-sm text-right"
                            value={newData.details[index].gold_rate}
                            onChange={handleItemsChange}
                            placeholder="Gold Rate"
                            onKeyDown={(event) => {
                              if (event.key === 'Enter') {
                        ...
                        <td>
                          <input
                            ...
                            data-id={index}
                            className="form-control form-control-sm text-right"
                            value={newData.details[index].la_cost}
                            onChange={handleItemsChange}
                            placeholder="LC"
                          />
                        </td>
                        <td>
                          <input
                            type="number"
                            name="wastage_per_pound"
                            id="wastage_per_pounds"
                            step="0.01"
                            data-id={index}
                            className="form-control form-control-sm text-right"
                            value={newData.details[index].wastage_per_pound}
                            onChange={handleItemsChange}
                            onKeyDown={(event) => {
                              if (event.key === 'Enter') {
                                addCostForWastageAsIndex(
                                  newData.details[index],
                                  event,
                                );
                              }
                        ...
                        <td>
                          <input
                            ...
                            data-id={index}
                            className="form-control form-control-sm text-right"
                            value={newData.details[index].tot_wastage}
                            onChange={handleItemsChange}
                            placeholder="Total Wastage"
                          />
                        </td>
                  ...
                  <tr>
                    ...
                    <td>
                      <input
                        ...
                        className="form-control-plaintext text-right kinda-important-text-3"
                        id="total_gold_rate"
                        name="total_gold_rate"
                        value={newData.total_gold_rate}
                        readOnly
                      />
                    </td>
                    <td>
                    ...
                    <td>
                      <input
                        type="text"
                        className="form-control-plaintext text-right kinda-important-text-3"
                        id="total_wastage_per_pound"
                        name="total_wastage_per_pound"
                        value={newData.total_wastage_per_pound}
                        readOnly
                      />
                    </td>
                    <td>
                      <input
                        type="text"
                        className="form-control-plaintext text-right kinda-important-text-3"
                        id="total_tot_wastage"
                        name="total_tot_wastage"
                        value={newData.total_tot_wastage}
                        readOnly
                      />
                    </td>
                    <td>
              ...
              <table className="table table-bordered table-sm table-hover">
                <thead className="thead-dark text-center">
                  ...
                  {/* rowSpan="2" */}
                    ...
                    <td width="25%">Metal Type</td>
                    <td width="25%">Gold Type</td>
                    <td width="25%">Weight (g)</td>
                    <td width="5%"></td>
                  </tr>
                  <tr>
                    <td>
                    <SDD
                        ...
                        placeholder="Metal"
                        listId="metal_types"
                        selected={itemMetalRow.metal.description}
                      />
                    </td>
                    <td>
                      <select
                        ...
                        className="form-control form-control-sm"
                        required
                        onChange={handleItemMetalRowChanges}
                        value={itemMetalRow.gold_type}
                      >
                        <option value="">---</option>
                        {goldTypes.map((obj) => {
                          return (
                ...
                <tbody> 
                  {newData.metal_details.map((item, index) => {
                    return (
                      <tr key={item.index}>
                        <th scope="row">{parseInt(index) + 1}</th>
                        <td>
                          <input
                            ...
                            data-id={index}
                            className="form-control-plaintext form-control-sm"
                            value={newData.metal_details[index].metal.description}
                            readOnly
                          />
                        </td>
                        <td>
                          <input
                            ...
                            data-id={index}
                            className="form-control-plaintext form-control-sm"
                            value={newData.metal_details[index].gold_type}
                            readOnly
                          />
                        </td>
                        <td>
                          <input
                            ...
                            data-id={index}
                            className="form-control form-control-sm text-right"
                            value={newData.metal_details[index].weight}
                            onChange={handleItemsChange}
                            placeholder="Weight"
                          />
                        </td>
                        <td className="text-center">
                          <SystemButton
                            type={'remove-row'}
                            method={() =>
                              removeItem(newData.metal_details[index].index)
                            }
                            showText={false}
                          />
                        </td>
                  ...
                  <tr>
                    ...
                    <td>
                      <input
                        ...
                        id="total_weight"
                        name="total_weight"
                        value={newData.melt_total_weight}
                        readOnly
                      />
                    </td>
                    <td></td>
