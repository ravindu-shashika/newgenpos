  ...
  const fetchData = async () => {
    try {
      ...
      const response = await api.get(
        `gem-pawning/${cookie.get('user_branch')}`,
      );
      ...
      if (isOldPawning) {
        ...
        if (response.data.message) {
          ...
          setNewLoan({
            ...
            required_amount: parseFloat(requiredAmt).toFixed(2),
          ...
          response.data[0].loan_item.forEach((item) => {
            itemsArr.push({
              ...
              gold_value: (
                parseFloat(
                  parseFloat(item.gold_weight) *
                    parseFloat(item.gold_rate.rate),
                ) / parseFloat(8.0)
              ).toFixed(2),
