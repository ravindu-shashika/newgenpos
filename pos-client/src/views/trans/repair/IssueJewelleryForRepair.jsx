...
import IssueJewelleryForRepairA4Half from '../../../printouts/IssueJewelleryForRepairA4Half';

const IssueJewelleryForRepair = () => {
  // Module name
  const moduleName = 'Issue Jewellery For Repair';
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await api.get(`issue-jewellery-repair`);
  ...
  const getRepairDetails = async () => {
    try {
      ...
      // newData.repair_no
      const response = await api.get(
        `get-repair-detailss/${newData.repair_no}/${newData.bc_no}`,
      );
      ...
      if(response.data.customer_repair){
        ...
        msg.warning('this item already place to repair');
        fetchData();
      }
  ...
  const save = async () => {
    try {
      const response = await api.post('issue-jewellery-repair').values(newData);
  ...
  const printReceipt = (trans_no, is_dupplicate) => {
    IssueJewelleryForRepairA4Half.loadInvoice(trans_no, cookie.get('user_branch'), is_dupplicate);
  };
...
export default IssueJewelleryForRepair;
