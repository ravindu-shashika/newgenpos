...
const GoldSmithGoldIssue = () => { 
  // Module name
  const moduleName = 'Gold Smith Gold Issue';
  ...
  useEffect(() => {
    calConvertColumnTotals();
  }, [newData.convert_details]);
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await api.get(`goldsmith-gold-issue`);
  ...
  const edit = async () => {
    try {
      ...
      const response = await api.get(
        `goldsmith-gold-issue/${newData.nno}/${newData.bc_no}`,
      );
      ...
      const convert_details = response.data.result.convert_details.map((item) => {
        return { ...item, index: uuidv4() };
      });


      dataObj.details = details;
      dataObj.convert_details = convert_details;
  ...
  const save = async () => {
    try {
      if (!isEdit) {
        const response = await api.post('goldsmith-gold-issue').values(newData);
        ...
        const response = await api
          .put('goldsmith-gold-issue', newData.id)
          .values(newData);
  ...
  const calConvertColumnTotals= () => {
    let total_weight = 0;

    newData.convert_details.forEach((row) => {
      console.log(row);
      // prettier-ignore
      total_weight = parseFloat(parseFloat(total_weight) + parseFloat(row.convert_weight)).toFixed(3);
      // prettier-ignore-end
    });

    setNewData({
      ...newData,
      convert_total_weight: total_weight
    });
  };

  const printReceipt = (trans_no, is_dupplicate) => {
    ResellRepairPrintA4Half.load(
      trans_no,
    ...
    <div>
      ...
      {isLoading.init ? (
        ...
        <div>
          ...
          <div className="row">
            <div className="table-responsive header-fixed-scrollable col-sm-6">
              <h2>Issued Gold</h2>
            ...
            <div className="table-responsive header-fixed-scrollable col-sm-6">
              ...
              <table className="table table-bordered table-sm table-hover">
                <thead className="thead-dark text-center">
                  ...
                  <tr>
                    ...
                    <SDD
                        ...
                        placeholder="Metal"
                        listId="metal_types"
                        selected={convertItemRow.convert_metal.description}
                      />
                    </td>
                    <td>
                      <select
                        id="convert_gold_type"
                        name="convert_gold_type"
                        className="form-control form-control-sm"
                        required
                        onChange={handleConvertItemRowChanges}
                        value={convertItemRow.convert_gold_type}
                      >
                        <option value="">---</option>
                        {goldTypes.map((obj) => {
                          return (
                    ...
                    <td>
                      <input
                        ...
                        step="0.01"
                        className="form-control form-control-sm text-right"
                        value={convertItemRow.convert_weight}
                        onChange={handleConvertItemRowChanges}
                        placeholder="Weight"
                      />
                    </td>
                ...
                <tbody>
                  {newData.convert_details.map((item, index) => {
                    return (
                      <tr key={item.index}>
                        <th scope="row">{parseInt(index) + 1}</th>
                        <td>
                          <input
                            ...
                            data-id={index}
                            className="form-control-plaintext form-control-sm"
                            value={newData.convert_details[index].convert_metal.description}
                            readOnly
                          />
                        </td>
                        <td>
                          <input
                            ...
                            data-id={index}
                            className="form-control-plaintext form-control-sm"
                            value={newData.convert_details[index].convert_gold_type}
                            readOnly
                          />
                        </td>
                        <td>
                          <input
                            ...
                            data-id={index}
                            className="form-control form-control-sm text-right"
                            value={newData.convert_details[index].convert_weight}
                            onChange={handleItemsChange}
                            placeholder="Weight"
                          />
                        </td>
                        ...
                        <td className="text-center">
                          <SystemButton
                            type={'remove-row'}
                            method={() =>
                              removeConvertItem(newData.convert_details[index].index)
                            }
                            showText={false}
                          />
                        </td>
                  ...
                  <tr>
                    ...
                    <td>
                      <input
                        ...
                        id="total_weight"
                        name="total_weight"
                        value={newData.convert_total_weight}
                        readOnly
                      />
                    </td>
                    <td></td>
...
export default GoldSmithGoldIssue;
