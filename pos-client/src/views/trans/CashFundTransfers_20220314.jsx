...
const CashFundTransfers = () => {
  ...
  const handleValueChange = (e) => {
    ...
    // });
    if (isReceive) {
      if (inputName == 'transfer_id') {
        const trans = transfers.find((trans) => {
          return inputValue == trans.id;
        });
        if (trans) {
          setNewData({
            ...newData,
            amount: trans.amount,
            to_branch_id: trans.branch_id,
            user_id: cookie.get('user_id'),
            //to_branch_id: 0,
            [inputName]: inputValue,
          });
  ...
  const loadTransfers = async () => {
    const response = await api.get(
      `get-fund-transfers-not-received/${cookie.get('user_branch')}`,
    );
              ...
              <div className="col-sm-6">
                {isReceive ? (
                  <div className="form-group">
                    <label htmlFor="transfer_id">Transfer No.</label>
                    <select
                      type="text"
                      name="transfer_id"
                      id="transfer_id"
                      className="form-control form-control-sm"
                      value={newData.transfer_id}
                      onChange={handleValueChange}
                    >
                      <option value="">-- Select a transfer No</option>
                      {transfers.map((transfer) => (
                        <option value={transfer.id} key={transfer.id}>
                          Branch:{transfer.branch.name} Transfer No:{' '}
                          {transfer.id}
                        </option>
                      ))}
                    </select>
            ...
            <div className="row">
              <div className="col-sm-6">
                <div className="form-group">
                  ...
                  {isReceive ? 'From Branch' : 'To Branch'}
                  <select
                    type="text"
                    name="to_branch_id"
                    id="to_branch_id"
