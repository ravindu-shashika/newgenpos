...
const ForfeitPurchase = () => {
  // Module name
  const moduleName = 'Forfeit Purchase';
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await api.get(`forfeit-purchases`);
  ...
  const save = async () => {
    try {
      if (!isEdit) {
        const response = await api.post('forfeit-purchases').values(newData);
        ...
        //   .values(newData);
        const response = await api
          .put('forfeit-purchases', newData.id)
          .values(newData);
  ...
  const calColumnTotals = () => {
    ...
    newData.details.forEach((row) => {
      ...
      // prettier-ignore-end
      // prettier-ignore
      total_amount = parseFloat(parseFloat(total_amount) + parseFloat(row.cost)).toFixed(2);
      // prettier-ignore-end
      // prettier-ignore
  ...
  const printReceipt = (trans_no, is_dupplicate) => {
    ForfeitPurchasePrintA4Half.load(
      trans_no,
                    ...
                    return (
                      <tr key={item.index}>
                        ...
                        <td>
                          <input
                            className="form-control form-control-sm text-right"
                            checked={newData.details[index].is_melt}
                            value="1"
                            type="checkbox"
                            id="is_melt"
                            name="is_melt"
...
export default ForfeitPurchase;
