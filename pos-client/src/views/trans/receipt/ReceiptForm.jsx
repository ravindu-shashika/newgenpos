...
import PaymentReceiptA4Half from '../../../printouts/PaymentReceiptA4Half';
import Cookies from 'universal-cookie';
import { useNavigate, useParams } from 'react-router-dom';

const ReceiptForm = () => {
  // Module name
  const moduleName = 'Receipt'; 
  ...
  const [isDisabledBank, setIsDisabledBank] = useState(false);
  const [isDisabledBankBranch, setIsDisabledBankBranch] = useState(false);
  ...
  const save = async () => {
    try {
      if (!isEdit) {
        ...
        const response = await api.put('receipt', newData.nno).values(newData);

        if (parseInt(response.data) > 0) {
          printReceipt(response.data, false);
          fetchData();
          msg.success('Voucher Saved Successfully.');
          navigate('/receipt/:nno/:bc_no');
  ...
  const checkBeforeSave = () => {
    ...
    if (newData.type === '') {
      msg.warning('Select payment type before save.');
      return false;
    }

    if (newData.paid_acc === '') {
        msg.warning('Select account before save.');
        return false;
      }
  ...
  const set_paying_acc = async () => {
    try {
      const response = await api
        ...
        if(newData.type == "Cash"){
            setIsDisabled(false);
            setIsDisabledBank(true);
            setIsDisabledBankBranch(true);

            setPayingAcount(response.data.account);
            setBankAccount([]);
            setBankAccountBranch([]);
        }else if(newData.type == "Cheque"){
            setPayingAcount([]);
            setBankAccount(response.data.account);
            setIsDisabled(true);
            setIsDisabledBank(false);
            setIsDisabledBankBranch(false);
        }
    } catch (err) {
      msg.error(err);
      return;
  ...
  const edit = async () => {
    try {
      ...
      const response = await api.get(
        `receipts/${newData.nno}/${newData.bc_no}`,
      );
  ...
  const getBankBranch =  async (value) => {
    try {
       
  
        // const response = await api.post(`get-bank-branch/${newData.nno}`);

        const response = await api.get(
            `get-bank-branch/${value}`,
          );

          setBankAccountBranch(response.data.bank_branch);
              ...
              <div className="row">
                ...
                <div className="col-sm-4 pr-0">
                  <select
                    ...
                    disabled={isDisabledBankBranch}
                    // value={newData.account_id}
                  >
                    <option value="">Select</option>
                    {bankAccountBranch.map((obj) => {
                      return (
                        <option key={obj.code} value={obj.code}>
                          {obj.code} - {obj.description}
                        </option>
