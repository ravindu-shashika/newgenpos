...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const apiStatus = body.status ?? response?.status;
    ...
    if (apiStatus == 200 && body.token) {
      ...
      (body.error ?? []).forEach((err) => msg.error(err));
      if (!body.error?.length) msg.error(body.message || 'Server error');
    } else {
      msg.error(body.message || 'Something went wrong. Check API URL and that Laravel is running.');
    }
  };
