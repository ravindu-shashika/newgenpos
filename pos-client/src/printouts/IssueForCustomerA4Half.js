...
const IssueForCustomerA4Half = {
  async loadInvoice(invoice_id, bc_no, is_dupplicate) {
    let trans_url = 'issue-for-customer/get-details';
  ...
  printPDFInvoice(is_dupplicate) {
    ...
    doc.text(this.print_data.branch.name, 215, 10);
    doc.text(this.print_data.ddate, 215, 17);
    doc.setFontSize(14);
    doc.text('' + this.print_data.id, 215, 24);
    ...
    if (parseFloat(this.print_data.credit_amount)) {
      doc.text('ISSUE FOR CUSTOMER', 193, 35);
    } else {
      doc.text('ISSUE FOR CUSTOMER', 193, 35);
    }
    ...
    doc.setFontSize(11);
    if (is_dupplicate) {
      doc.text('DUPLICATE', 193, 40.5);
    } else {
      doc.text('ORIGINAL', 193, 40.5);
    }
    ...
    doc.text(
      this.print_data.customer.cusname + '(' + this.print_data.customer.nicno + ')',
      38,
      y,
    );
    ...
    doc.text('Design', 120, y, 'right');
    doc.text('Gold Type', 145, y, 'right');
    doc.text('Metal', 165, y, 'right');
    doc.text('Color', 185, y, 'right');
    doc.text('Gender', 205, y, 'right');
    doc.text('Weight', 225, y, 'right');
    doc.text('Cost', 247, y, 'right');
    ...
    this.print_data.details.forEach((item) => {
      ...
      doc.text(item.design.designname, 120, y, 'right');
      doc.text(item.gold_type, 145, y, 'right');
      doc.text(item.metal.description, 165, y, 'right');
      doc.text(item.color.description, 185, y, 'right');
      doc.text(item.gender.description, 205, y, 'right');
      doc.text(item.weight, 225, y, 'right');
      doc.text(item.cost, end_x, y, 'right');
...
export default IssueForCustomerA4Half;
