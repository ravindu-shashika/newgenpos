...
const ForfeitPurchasePrintA4Half = {
  async load(trans_no, bc_no, is_dupplicate) {
    const response = await api.get(`forfeit-purchases/${trans_no}/${bc_no}`);
  ...
  printPDF(is_dupplicate) {
    ...
    doc.text('Goods Received Note', 300, 35);   
...
export default ForfeitPurchasePrintA4Half;
