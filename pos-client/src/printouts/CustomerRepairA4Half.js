...
const CustomerRepairA4Half = {
  async loadInvoice(invoice_id, bc_no, is_dupplicate) {
    let trans_url = 'customer-repair/get-details';
    ...
    if (response.data.customer_repair) {
      this.print_data = response.data.customer_repair;
      this.total_items = response.data.customer_repair.details.length;
...
export default CustomerRepairA4Half;
