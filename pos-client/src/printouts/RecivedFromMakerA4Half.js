...
const RecivedFromMakerA4Half = {
  async loadInvoice(invoice_id, bc_no, is_dupplicate) {
    let trans_url = 'issue-for-maker/get-details';
  ...
  printPDFInvoice(is_dupplicate) {
    ...
    if (parseFloat(this.print_data.credit_amount)) {
      doc.text('RECIVED FROM MAKER', 193, 35);
    } else {
      doc.text('RECIVED FROM MAKER', 193, 35);
    }
    ...
    doc.text('Metal', 170, y, 'right');
    doc.text('Color', 190, y, 'right');
    doc.text('Gender', 210, y, 'right');
    doc.text('Weight', 230, y, 'right');
    doc.text('LCV', 247, y, 'right');
    ...
    this.print_data.details.forEach((item) => {
      ...
      doc.text(item.metal.description, 170, y, 'right');
      doc.text(item.color.description, 190, y, 'right');
      doc.text(item.gender.description, 210, y, 'right');
      doc.text(item.weight, 230, y, 'right');
      doc.text(item.lcv, end_x, y, 'right');
...
export default RecivedFromMakerA4Half;
