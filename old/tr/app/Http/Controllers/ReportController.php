...
class ReportController extends Controller
{
    ...
    public function profitLoss(Request $request)
    {
        ...
        if(isset($warehouse_id) && $warehouse_id != 0){
            $product_sale_data = $product_sale_data->where('sales.warehouse_id', $warehouse_id);
        }

        $product_sale_data = $product_sale_data->groupBy('product_sales.product_id', 'product_sales.product_batch_id')
                            ->get();
        ...
        if(isset($warehouse_id) && $warehouse_id != 0){
            $purchase = $purchase->where('warehouse_id', $warehouse_id);
        }
        $purchase = $purchase->selectRaw(implode(',', $query1))->get();
        ...
        if(isset($warehouse_id) && $warehouse_id != 0){
            $total_purchase = $total_purchase->where('warehouse_id', $warehouse_id);
        }
        ...
        if(isset($warehouse_id) && $warehouse_id != 0){
            $sale = $sale->where('warehouse_id', $warehouse_id);
        }
        $sale = $sale->selectRaw(implode(',', $query1))->get();
        ...
        if(isset($warehouse_id) && $warehouse_id != 0){
            $total_sale = $total_sale->where('warehouse_id', $warehouse_id);
        }
        ...
        $return = Returns::whereDate('created_at', '>=' , $start_date)
                        ->whereDate('created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $return = $return->where('warehouse_id', $warehouse_id);
        }
        $return = $return->selectRaw(implode(',', $query2))->get();

        $total_return = Returns::whereDate('created_at', '>=' , $start_date)
                            ->whereDate('created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $total_return = $total_return->where('warehouse_id', $warehouse_id);
        }
        ...
        $purchase_return = ReturnPurchase::whereDate('created_at', '>=' , $start_date)
                                        ->whereDate('created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $purchase_return = $purchase_return->where('warehouse_id', $warehouse_id);
        }
        $purchase_return = $purchase_return->selectRaw(implode(',', $query2))->get();

        $total_purchase_return = ReturnPurchase::whereDate('created_at', '>=' , $start_date)
                                            ->whereDate('created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $total_purchase_return = $total_purchase_return->where('warehouse_id', $warehouse_id);
        }
        ...
        $total_expense = Expense::whereDate('created_at', '>=' , $start_date)
                            ->whereDate('created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $total_expense = $total_expense->where('warehouse_id', $warehouse_id);
        }
        $total_expense = $total_expense->count();
        ...
        $payment_recieved_query = DB::table('payments')
                                    ->join('sales', 'payments.sale_id', '=', 'sales.id')
                                    ->whereNotNull('payments.sale_id')
                                    ->whereDate('payments.created_at', '>=' , $start_date)
                                    ->whereDate('payments.created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $payment_recieved_query = $payment_recieved_query->where('sales.warehouse_id', $warehouse_id);
        }
        $payment_recieved_number = (clone $payment_recieved_query)->count();
        $payment_recieved = (clone $payment_recieved_query)->sum(DB::raw('payments.amount / payments.exchange_rate'));

        // ================== CREDIT CARD PAYMENT SALE WITH WAREHOUSE FILTER ==================
        $credit_card_payment_sale = DB::table('payments')
                                    ...
                                    ->whereDate('payments.created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $credit_card_payment_sale = $credit_card_payment_sale->where('sales.warehouse_id', $warehouse_id);
        }
        $credit_card_payment_sale = $credit_card_payment_sale->sum(DB::raw('payments.amount / payments.exchange_rate'));

        // ================== CHEQUE PAYMENT SALE WITH WAREHOUSE FILTER ==================
        $cheque_payment_sale = DB::table('payments')
                                ...
                                ->whereDate('payments.created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $cheque_payment_sale = $cheque_payment_sale->where('sales.warehouse_id', $warehouse_id);
        }
        $cheque_payment_sale = $cheque_payment_sale->sum(DB::raw('payments.amount / payments.exchange_rate'));

        // ================== GIFT CARD PAYMENT SALE WITH WAREHOUSE FILTER ==================
        $gift_card_payment_sale = DB::table('payments')
                                    ...
                                    ->whereDate('payments.created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $gift_card_payment_sale = $gift_card_payment_sale->where('sales.warehouse_id', $warehouse_id);
        }
        $gift_card_payment_sale = $gift_card_payment_sale->sum(DB::raw('payments.amount / payments.exchange_rate'));

        // ================== PAYPAL PAYMENT SALE WITH WAREHOUSE FILTER ==================
        $paypal_payment_sale = DB::table('payments')
                                ...
                                ->whereDate('payments.created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $paypal_payment_sale = $paypal_payment_sale->where('sales.warehouse_id', $warehouse_id);
        }
        $paypal_payment_sale = $paypal_payment_sale->sum(DB::raw('payments.amount / payments.exchange_rate'));

        // ================== DEPOSIT PAYMENT SALE WITH WAREHOUSE FILTER ==================
        $deposit_payment_sale = DB::table('payments')
                                ...
                                ->whereDate('payments.created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $deposit_payment_sale = $deposit_payment_sale->where('sales.warehouse_id', $warehouse_id);
        }
        $deposit_payment_sale = $deposit_payment_sale->sum(DB::raw('payments.amount / payments.exchange_rate'));
        ...
        // ================== PAYMENT SENT QUERIES WITH WAREHOUSE FILTER ==================
        $payment_sent_query = DB::table('payments')
                                ->join('purchases', 'payments.purchase_id', '=', 'purchases.id')
                                ->whereNotNull('payments.purchase_id')
                                ->whereDate('payments.created_at', '>=' , $start_date)
                                ->whereDate('payments.created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $payment_sent_query = $payment_sent_query->where('purchases.warehouse_id', $warehouse_id);
        }
        $payment_sent_number = (clone $payment_sent_query)->count();
        $payment_sent = (clone $payment_sent_query)->sum(DB::raw('payments.amount / payments.exchange_rate'));

        // ================== CREDIT CARD PAYMENT PURCHASE WITH WAREHOUSE FILTER ==================
        $credit_card_payment_purchase = DB::table('payments')
                                        ...
                                        ->whereDate('payments.created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $credit_card_payment_purchase = $credit_card_payment_purchase->where('purchases.warehouse_id', $warehouse_id);
        }
        $credit_card_payment_purchase = $credit_card_payment_purchase->sum(DB::raw('payments.amount / payments.exchange_rate'));

        // ================== CHEQUE PAYMENT PURCHASE WITH WAREHOUSE FILTER ==================
        $cheque_payment_purchase = DB::table('payments')
                                    ...
                                    ->whereDate('payments.created_at', '<=' , $end_date);
        if(isset($warehouse_id) && $warehouse_id != 0){
            $cheque_payment_purchase = $cheque_payment_purchase->where('purchases.warehouse_id', $warehouse_id);
        }
        $cheque_payment_purchase = $cheque_payment_purchase->sum(DB::raw('payments.amount / payments.exchange_rate'));
