                                                                ...
                                                                @foreach ($lims_product_sale_data as $key => $product_sale)
                                                                    <tr>
                                                                        ...
                                                                        <input type="hidden" name="type[]"
                                                                            value="return">
                                                                        <td>{{ $product_data->name ?? 'N/A' }} <br> {{ $product_data->code ?? 'N/A' }}</td>
...
@push('scripts')
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            ...
                            if (data.length === 1) {
                                if (click === 0) {
                                    if (type == 'return') {
                                        $('#sale_product-results-container .product-img').first()
                                            .trigger('click');
                                    } else {
    ...
    <script type="text/javascript">
        ...
        function productSearch(data) {
            ...
            // ✅ আগেই table selector decide করা
            ...
            $(".product-code").each(function(i) {
                if ($(this).val().trim() == item_code) {
                    ...
                    if (data.imei != 'null' && data.imei != '') {
                        // ✅ tableSelector use করা এবং undefined check করা
                        imeiNumbers = $(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ') .imei-number')
                            .val();

                        // ✅ imeiNumbers exist এবং empty না - তারপর split করা
            ...
            if (flag) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        // tableSelector আগেই define করা আছে, তাই আবার করার দরকার নেই
                        ...
                        if (pre_qty > 0) {
                            ...
                            product_price[rowindex] =
                                parseFloat(data[2] * currency['exchange_rate']) +
                        ...
                        if (flag) {
                            ...
                            $(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ')')
        ...
        function addNewProduct(data, tableSelector) { // ✅ tableSelector parameter যোগ করলাম
            console.log('this is a table section function: ' + data + '<br>' + tableSelector)
            ...
            cols += '<input type="hidden" name="type[]" value="new">';
            ...
            // ✅ এখানেও tableSelector use করলাম
            $(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ')').find('.product_price').val(product_price[
                rowindex]);
            ...
            <div class="modal fade" id="productSelectionModal" tabindex="-1" role="dialog" aria-labelledby="productSelectionModalLabel" aria-hidden="true" data-rowindex="${rowIndex}">
                <div class="modal-dialog" role="document">
                    ...
                    $("#confirmSelection").on('click', function() {
                            ...
                            if (product_discount[rowindex] < 1) {
                                // ✅ এখানেও tableSelector use করলাম
                                cur_product_id = $(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) +
                                    ') .product-id').val();
                                @if (isset($draft_product_discount))
                                    if (product_discount[rowindex] < 1) {
                                        draft_discounts = @json($draft_product_discount['discount']);
                            ...
                            $("input[name='productOption']:checked").each(function() {
                                const qty = parseFloat($(`#quantity_${$(this).val()}`).val() || 1);
            ...
            if (imeiNumbers.length) {
                ...
                for (var i = 0; i < imeiArrays.length; i++) {
                    htmlText +=
                        ` < tr >
                                    ...
                                    <
                                    input type = "text"
                                    class = "form-control imei-numbers"
                                    name = "imei_numbers[]"
                                    value = "` +
                                imeiArrays[i] + `" />
                                </td>
                    ...
                    populatePriceOption(); $("#product-cost").text(cost[rowindex]);
                    var row_product_name_code = $('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')')
                        .find(
                            'td:nth-child(1) > strong:nth-child(1)').text(); $('#modal_header').text(
                        row_product_name_code);

                    var qty = $('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.qty')
                .val(); $('input[name="edit_qty"]').val(qty);

                    cur_product_id = $('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ') .product-id')
                    .val();
                    @if (isset($draft_product_discount))
                        if (product_discount[rowindex] < 1) {
                            draft_discounts = @json($draft_product_discount['discount']);
                    ...
                    var tax_name_all = <?php echo json_encode($tax_name_all); ?>; pos = tax_name_all.indexOf(tax_name[rowindex]); $(
                    ...
                    if (product_type == 'standard') {
                        ...
                        $.each(temp_unit_name, function(key, value) {
                            $('select[name="edit_unit"]').append('<option data-operator="' +
                                temp_unit_operator[key] +
                                '" data-operation-value="' + temp_unit_operation_value[key] +
                                '" value="' + key + '">' +
                                value + '</option>');
                        });
                        $("#edit_unit").show();
                    ...
                    $('input[name="edit_unit_price"]').val(row_product_price.toFixed(
                        {{ $general_setting->decimal }})); $('.selectpicker').selectpicker('refresh');
                }
                ...
                function checkDiscount(qty, flag, tableSelector = 'table.order-list', price = 0) {
                    ...
                    $.ajax({
                        type: 'GET',
                        async: false,
                        url: '{{ url('/') }}/sales/check-discount?qty=' + qty + '&customer_id=' +
                            customer_id +
                            '&product_id=' + product_id + '&warehouse_id=' + warehouse_id,
                        success: function(data) {
                            if (product_price[rowindex].length == 0) {
                                product_price[rowindex] = $(tableSelector + ' tbody tr:nth-child(' + (
                                        rowindex + 1) +
                                    ') .product_price').val();
                            }
                            product_price[rowindex] = parseFloat(product_price[rowindex] * currency[
                                    'exchange_rate']) +
                                parseFloat(product_price[rowindex] * currency['exchange_rate'] *
                                    customer_group_rate);
                            ...
                            if (flag == true)
                                ...
                                $('#discount').text(productDiscount - data[2] * previousqty + data[2] *
                                qty);
                            else
                                $('#discount').text(productDiscount - data[2]);
                        }
                ...
                function checkQuantity(sale_qty, flag, tableSelector =
                'table.order-list') { // ✅ tableSelector parameter যোগ
                    var row_product_code = $(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ')').find(
                            '.product-code')
                        .val();
                    var qty = parseFloat($(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ')').find('.qty')
                        .attr('max'));
                    var product_type = $(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ')').find(
                        '.product_type').val();

                    if (without_stock == 'no') {
                        // ✅ product_type undefined check যোগ করা
                        if (product_type && (product_type.trim() == 'standard' || product_type.trim() == 'combo')) {
                            ...
                            if (total_qty > qty) {
                                if (imeiNumbers.length) {
                                    ...
                                    if (flag) {
                                        sale_qty = (sale_qty - 1);
                                        checkQuantity(sale_qty, true, tableSelector); // ✅ tableSelector pass করা
                                    } else {
                                        edit();
                                        return;
                                    }
                            ...
                            $(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ')').find('.qty').val(
                                sale_qty); // ✅ dynamic
                        }
                    } else
                        $(tableSelector + ' tbody tr:nth-child(' + (rowindex + 1) + ')').find('.qty').val(
                ...
                function calculateRowProductData(quantity) {
                    ...
                    if (tax_method[rowindex] == 1) {
                        ...
                        var topping_price = ($('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find(
                                '.topping-price')
                            .val() * quantity) || 0;
                        ...
                        $('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.tax-value').val(tax.toFixed(
                            {{ $general_setting->decimal }}));
                        $('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.product-price').text(
                            sub_total_unit
                            .toFixed({{ $general_setting->decimal }}));
                    ...
                    function calculateGrandTotal() {
                        ...
                        if (!currencyChange)
                            ...
                            var shipping_cost = parseFloat($('input[name="shipping_cost"]').val() * currency[
                                'exchange_rate']);
                        ...
                        if (!currencyChange)
                            ...
                            var coupon_discount = parseFloat($('input[name="coupon_discount"]').val() * currency[
                                'exchange_rate']);
                        if (!coupon_discount)
                    ...
                    function toggleExchange(el) {
                     ...
                     var originalCalculateTotal = window.calculateTotal;
