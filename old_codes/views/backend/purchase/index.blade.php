...
<script type="text/javascript">
    ...
    $(document).on("click", "table.purchase-list tbody .add-payment", function(event) {
        ...
        // if(show_purchase_product_details == 1){
        //     var balance = $('table.purchase-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('td:nth-child(12)').text();
        // }else{
        //     var balance = $('table.purchase-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('td:nth-child(10)').text();
        // }
        ...
        $.get('purchases/getpayment/' + id, function(data) {
            ...
            $.each(payment_date, function(index){
                ...
                if(all_permission.indexOf("purchase-payment-delete") != -1)
                    cols += '<form action="{{ route('purchase.delete-payment') }}" method="POST">'+'@csrf'+'<li><input type="hidden" name="id" value="' + payment_id[index] + '" /> <button type="submit" class="btn btn-link" onclick="return confirmDeletePayment()"><i class="dripicons-trash"></i> Delete</button></li></form>';
                cols += '</ul></div></td>';
                newRow.append(cols);
    ...
    if (show_purchase_product_details == 1) {
        targets = [0, 3, 4, 5, 6, 7, 9, 11, 12, 13, 14];
    } else {
        targets = [0, 3, 4, 5, 7, 9, 10, 11, 12];
    }
    ...
    function purchaseDetails(purchase){
        ...
        if(purchase[28])
            htmltext += 'Payment Term: '+purchase[28]+' '+purchase[29]+'<br>';
        if(purchase[30])
            htmltext += 'Due Date: '+purchase[30]+'<br>';
        ...
        $.get('purchases/product_purchase/' + purchase[3], function(data) {
            ...
            if(data == 'Something is wrong!') {
                ...
                // returned_amount এখন [31]
                if(purchase[31] != 0){
                    var newRow = $("<tr>");
                    cols = '<td colspan=8>{{__("db.Return Amount")}}:</td>';
                    cols += '<td>' + purchase[31] + '</td>';
                    newRow.append(cols);
                    newBody.append(newRow);
                }
                ...
                cols += '<td>' + (purchase[20] - purchase[31] - purchase[21]) + '</td>';
                newRow.append(cols);
                newBody.append(newRow);
