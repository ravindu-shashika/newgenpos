                        ...
                        <div class="row no-gutters">
                            ...
                            <div class="col-auto border-right" style="width:230px;min-height:650px;background:#fafafa;">
                                <div class="p-3 border-bottom">
                                    <button class="btn btn-primary btn-block" id="addBookingBtn">
                            ...
                            <div class="col-md-4 mt-3">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="daterangepicker-field form-control" value="{{ \Carbon\Carbon::parse($starting_date)->format($general_setting->date_format) }} To {{ \Carbon\Carbon::parse($ending_date)->format($general_setting->date_format) }}" />
                        ...
                        <table id="booking-table" class="table booking-list">
                            ...
                            <tbody>
                                ...
                                <tr data-id="{{ $booking->id }}">
                                    <td>{{ $key }}</td>
                                    <td>{{ $booking->created_at->format($general_setting->date_format) }}</td>
                                    ...
                                    <td>{{ $booking->price ?? '—' }}</td>
                                    <td>{{ \Carbon\Carbon::parse($booking->start_date)->format($general_setting->date_format . ' H:i') }}</td>
                                    <td>{{ \Carbon\Carbon::parse($booking->end_date)->format($general_setting->date_format . ' H:i') }}</td>
                                    <td>
            ...
            <div class="modal-body">
                ...
                <form id="bookingForm" method="POST" action="{{ route('bookings.store') }}">
                    ...
                    <div class="row">
                        ...
                        {{-- Start Date & Time — plain input, click to open picker --}}
                        <div class="col-md-6 form-group">
                            <label>{{ __('db.Start Date') }} & Time *</label>
                        ...
                        {{-- End Date & Time — plain input, click to open picker --}}
                        <div class="col-md-6 form-group">
                            <label>{{ __('db.End Date') }} & Time *</label>
...
<style>
    ...
    /* Input click cursor */
    #b_start_date, #b_end_date { cursor: pointer; background: #fff; }

    /* Picker must float above modal overlay (z-index 1050) */
...
<script>
var dateFormat     = '{{ $general_setting->date_format }}';
var datetimeFormat = dateFormat.toUpperCase() + ' HH:mm';
...
$(document).ready(function () {
    ...
    // ── DateTime Pickers ──────────────────────────────────────────
    // widgetParent: 'body'  → body-তে render হবে, modal-এর z-index block করবে না
    // inline: false         → dropdown হিসেবে খুলবে
    // sideBySide: false     → overlap হবে না
    // useCurrent: false     → resetModal()-এ manually set করা হবে
    var dtpOptions = {
    ...
    $('#b_start_date').datetimepicker(dtpOptions);
    $('#b_end_date').datetimepicker(dtpOptions);

    // Input-এ click করলে picker open হবে
    $('#b_start_date').on('click', function () { $(this).datetimepicker('show'); });
    $('#b_end_date').on('click',   function () { $(this).datetimepicker('show'); });
    ...
    var calendar = new FullCalendar.Calendar(document.getElementById('calendar'), {
        ...
        loading: function (isLoading) {
            isLoading ? $('#calendarLoader').fadeIn(200) : $('#calendarLoader').fadeOut(300);
        },
        ...
        // dateClick এখন কাজ করবে কারণ datetimepicker widgetParent:'body' তে আছে
    ...
    function resetModal() {
        ...
        $('#bookingModalLabel').text('{{ __("db.Add Booking") }}');

        $('#b_start_date').datetimepicker('clear');
        $('#b_end_date').datetimepicker('clear');
    ...
    function openAddForm(dateStr) {
        resetModal();
        if (dateStr) {
            $('#b_start_date').datetimepicker('date', moment(dateStr, 'YYYY-MM-DD').hour(9).minute(0));
            $('#b_end_date').datetimepicker('date', moment(dateStr, 'YYYY-MM-DD').hour(17).minute(0));
        }
        $('#booking-modal').modal('show');
    }

    // ── Edit from calendar event ──────────────────────────────────
    function openEditForm(event) {
        ...
        if (event.start) $('#b_start_date').datetimepicker('date', moment(event.start));
        if (event.end)   $('#b_end_date').datetimepicker('date', moment(event.end));
    ...
    function openEditFromData(b) {
        ...
        if (b.start_date) $('#b_start_date').datetimepicker('date', moment(b.start_date));
        if (b.end_date)   $('#b_end_date').datetimepicker('date', moment(b.end_date));
    ...
    $('#bookingForm').on('submit', function (e) {
        ...
        if (!$('#b_start_date').val())   { e.preventDefault(); alert('Please select Start Date & Time!'); return; }
        if (!$('#b_end_date').val())     { e.preventDefault(); alert('Please select End Date & Time!');   return; }

        var startMoment = moment($('#b_start_date').val(), datetimeFormat);
        var endMoment   = moment($('#b_end_date').val(),   datetimeFormat);

        if (!startMoment.isValid()) { e.preventDefault(); alert('Start date is invalid!'); return; }
        if (!endMoment.isValid())   { e.preventDefault(); alert('End date is invalid!');   return; }
        ...
        // DB format-এ convert করে send করো
        $('#b_start_date').val(startMoment.format('YYYY-MM-DD HH:mm:ss'));
        $('#b_end_date').val(endMoment.format('YYYY-MM-DD HH:mm:ss'));
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    location.href = '{{ route("booking.index") }}?tab=list';
                }
            }
        });
    });
