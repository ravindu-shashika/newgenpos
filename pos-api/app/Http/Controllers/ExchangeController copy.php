...
class ExchangeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $lims_sale_data = Sale::whereNull('deleted_at')->find($data['sale_id']);
            ...
            foreach ($product_id as $index => $id) {
                ...
                if ($product_type === 'return') {
                    ...
                    // Check if this product should be returned
                    if ($product_code_value && in_array($product_code_value, $is_exchange)) {
                        // Get original product sale data
                        ...
                        // Process as RETURN - Add to Return table
                        $this->processReturnToReturnTable(
                            $id,
                            $index,
                            $lims_exchange_data,
                            $lims_sale_data,
                ...
                elseif ($product_type === 'new') {
                    // Process as NEW - Add to Original Sale
                    $this->processNewToSaleTable(
                        $id,
                        $index,
                        $lims_exchange_data,
                        $lims_sale_data,
            ...
            $message = "Exchange created successfully with {$new_products_count} new product(s) added to sale and {$returned_products_count} product(s) returned";
    ...
    /**
     * Process NEW products - Add to Original Sale (like Sale Edit)
     */
    private function processNewToSaleTable(
        $product_id,
        $index,
        $lims_exchange_data,
        $lims_sale_data,
            ...
            if ($lims_sale_unit_data) {
                ...
                if ($lims_product_data->is_variant) {
                    ...
                    if ($lims_product_variant_data) {
                        ...
                        $lims_product_warehouse_data = ProductWarehouse::FindProductWithVariant(
                            $product_id,
                ...
                elseif ($product_batch_id[$index]) {
                    ...
                    $product_batch_data = ProductBatch::find($product_batch_id[$index]);
                ...
                // Regular product
                    ...
                    $lims_product_warehouse_data = ProductWarehouse::FindProductWithoutVariant(
                        $product_id,
                        $lims_sale_data->warehouse_id
                    )->first();
                }
        ...
        // Add to Product_Sale table (adding to original sale)
        $product_sale['sale_id'] = $lims_sale_data->id;
        $product_sale['product_id'] = $product_id;
        $product_sale['product_batch_id'] = $product_batch_id[$index] ?? null;
        $product_sale['qty'] = $qty[$index];
        $product_sale['sale_unit_id'] = $sale_unit_id;
        $product_sale['net_unit_price'] = $net_unit_price[$index];
        $product_sale['discount'] = $discount[$index];
        $product_sale['tax_rate'] = $tax_rate[$index];
        $product_sale['tax'] = $tax[$index];
        $product_sale['total'] = $total[$index];

        if ($imei_number[$index] && !str_contains($imei_number[$index], "null")) {
            $product_sale['imei_number'] = $imei_number[$index];
        } else {
        ...
        // Update Sale totals
        $lims_sale_data->total_qty += $qty[$index];
        $lims_sale_data->total_tax += $tax[$index];
        $lims_sale_data->total_price += $total[$index];
        $lims_sale_data->grand_total += $total[$index];
        $lims_sale_data->save();

        // Create product exchange record for tracking
        ProductExchange::create([
            'exchange_id' => $lims_exchange_data->id,
            'product_id' => $product_id,
            'qty' => $qty[$index],
    ...
    /**
     * Process RETURNED products - Add to Return table
     */
    private function processReturnToReturnTable(
        $product_id,
        $index,
        $lims_exchange_data,
        $lims_sale_data,
        ...
        // Create Return record if not exists for this exchange
        $lims_return_data = Returns::where('sale_id', $lims_sale_data->id)
            ->where('reference_no', 'LIKE', $lims_exchange_data->reference_no)
            ->first();

        if (!$lims_return_data) {
            $return_data = [
                'reference_no' => 'rr-ex-' . $lims_exchange_data->reference_no,
                'user_id' => Auth::id(),
                ...
                'order_tax' => 0,
                'grand_total' => 0,
                'return_note' => 'Auto-created from exchange: ' . $lims_exchange_data->reference_no,
                'staff_note' => 'Exchange reference: ' . $lims_exchange_data->reference_no,
                'account_id' => request()->account_id,
            ];

            $lims_return_data = Returns::create($return_data);
        }
        ...
        // Add to Product_Return table
        $product_return = [
            'return_id' => $lims_return_data->id,
            'product_id' => $product_id,
            'product_batch_id' => $batch_id,
            'variant_id' => $variant_id,
            'imei_number' => ($imei_number[$index] && !str_contains($imei_number[$index], "null")) ? $imei_number[$index] : null,
            'qty' => $qty[$index],
            'sale_unit_id' => $sale_unit_id,
            'net_unit_price' => $net_unit_price[$index],
            'discount' => $discount[$index],
        ...
        $lims_return_data->item += 1;
        $lims_return_data->total_qty += $qty[$index];
        $lims_return_data->total_tax += $tax[$index];
        $lims_return_data->total_price += $total[$index];
        $lims_return_data->grand_total += $total[$index];
