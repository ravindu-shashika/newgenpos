...
class ProductController extends Controller
{
        ...
        try {
            ...
            if(isset($data['is_variant'])) {
                ...
                foreach ($data['variant_name'] as $key => $variant_name) {
                    ...
                    $product_warehouses = ProductWarehouse::where([
                        'product_id' => $lims_product_data->id,
                        'variant_id' => $lims_variant_data->id
                    ])->get();
            ...
            $data['alt_code'] = $request->input('alt_code');
    ...
    public function productWarehouseData($id)
    {
        ...
        foreach ($lims_product_warehouse_data as $key => $product_warehouse_data) {
            ...
            $imeis = ProductWarehouse::select('imei_number')
    ...
    public function uploadGalleryImages(Request $request)
    {
        ...
        foreach ($files as $file) {
            ...
            $file->storeAs('public/product-gallery', $filename);

            $stored[] = [
                'filename' => $filename,
                'url' => asset('storage/product-gallery/' . $filename),
            ];
        }
    ...
    {
        $productCodes = $this->productWithoutVariant()
            ->map(fn ($product) => [
        ...
        $variantCodes = $this->productWithVariant()
            ->map(fn ($product) => [
        ...
        $barcodeSettings = Barcode::select(
            'id',
            'name',
            'description',
            'is_default',
        ...
        $defaultSetting = $barcodeSettings->firstWhere('is_default', true);
        ...
        return response()->json([
            'suggestions' => $productCodes->concat($variantCodes)->values(),
            'barcode_settings' => $barcodeSettings,
            'default_barcode_setting_id' => $defaultSetting?->id,
    ...
    public function searchProducts(Request $request)
    {
        ...
        $variantProducts = Product::join('product_variants', 'products.id', '=', 'product_variants.product_id')
            ...
            ->where(function ($q) use ($query) {
                $q->where('products.name', 'like', '%' . $query . '%')
                    ->orWhere('product_variants.item_code', 'like', '%' . $query . '%');
            })
        ...
        $results = $products->concat($variantProducts)->take($limit)->values();
    ...
    {
        $codeInput = $request->input('code', $request->input('data', ''));
        ...
        $product_code = explode('(', $codeInput);
        ...
        if ($productList->isEmpty()) {
            $productList = Product::join('product_variants', 'products.id', '=', 'product_variants.product_id')
                ->select('products.*', 'product_variants.item_code', 'product_variants.variant_id', 'product_variants.additional_price')
                ->where('product_variants.item_code', $product_code[0])
                ->get();
        } elseif ($productList->first()->is_variant) {
            $productList = Product::join('product_variants', 'products.id', '=', 'product_variants.product_id')
                ->select('products.*', 'product_variants.item_code', 'product_variants.variant_id', 'product_variants.additional_price')
                ->where('product_variants.product_id', $productList->first()->id)
                ->get();
        }
        ...
        foreach ($productList as $product) {
            ...
            $variant_id = $product->is_variant ? $product->variant_id : null;
            $additional_price = $product->is_variant ? ($product->additional_price ?? 0) : 0;

            $brand = $product->brand_id ? Brand::find($product->brand_id) : null;

            $warehousePrices = collect();
            $hasDifferentPrice = false;

            if ($request->boolean('barcode', true)) {
                $warehousePrices = ProductWarehouse::select(
                    'product_warehouses.price',
                    ...
                    ->whereNotNull('product_warehouses.price')
                    ->latest()
                    ->get();

                $hasDifferentPrice = $warehousePrices->contains(function ($wp) use ($product) {
                    return $product->price != $wp->price;
                });
            }

            $unitCollection = Unit::query()
                ->where('id', $product->unit_id)
                ->orWhere('base_unit', $product->unit_id)
                ->get()
                ->unique('id');

            $unitOptions = $unitCollection->map(fn ($unit) => [
                'id' => $unit->id,
            ...
            $responseProducts[] = [
                ...
                'brand_name' => $brand->title ?? 'N/A',
                'unit_id' => $product->unit_id,
                'unit_options' => $unitOptions,
                'default_price' => round($product->price, 4),
                'price' => round($product->price + $additional_price, 4),
                'promotion_price' => $product->promotion_price,
                'cost' => $product->cost,
                'available_qty' => $product->qty,
                'currency' => config('currency'),
                'currency_position' => config('currency_position'),
                'barcode_image' => DNS1D::getBarcodePNG($code, $product->barcode_symbology ?? 'C128'),
                'has_different_price' => $hasDifferentPrice,
                'warehouse_prices' => $warehousePrices->map(fn ($item) => [
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        foreach($lims_product_list as $lims_product_data) {
            ...
            if($request->barcode == true){
                $warehouse_product = ProductWarehouse::select(
                    'product_warehouses.*',
                    'warehouses.name as warehouse_name' // warehouse এর name সহ
                )
                ->join('warehouses', 'product_warehouses.warehouse_id', '=', 'warehouses.id')
                ->where('product_warehouses.product_id', $lims_product_data->id)
                ->where('product_warehouses.price','!=',null)
                ->latest()
                ->get();
    ...
    public function importProduct(Request $request)
    {
        ...
        if ($ext != 'csv') {
            if ($request->expectsJson()) {
                return response()->json(['success' => false, 'message' => __('db.Please upload a valid CSV file')], 422);
            }
        ...
        if (!$header) {
            fclose($file);
            if ($request->expectsJson()) {
                return response()->json(['success' => false, 'message' => __('db.CSV file is empty or invalid')], 422);
            }
        ...
        try {
            ...
            while ($columns = fgetcsv($file)) {
                ...
                if (!empty($data['variantvalue']) && !empty($data['variantname'])) {
                    ...
                    if (config('without_stock') === 'yes') {
                        ProductWarehouse::insert($productWarehouses);
                    }
            ...
            if ($request->expectsJson()) {
                return response()->json([
                    'success' => true,
                    'message' => __('db.Products imported successfully!')
                ]);
            }
    ...
    public function allProductInStock()
    {
        if(!in_array('ecommerce',explode(',', config('addons'))))
        {
            if (request()->expectsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => __('db.Please install the ecommerce addon!')
                ], 422);
            }
        ...
        if (request()->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => __('db.All Products set to in stock successfully!')
            ]);
        }
    ...
    public function showAllProductOnline()
    {
        ...
        if (request()->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => __('db.All Products will be showed to online!')
            ]);
        }
    ...
    public function getProduct($id)
    {
        try {
            ...
            if ($product->variant_option && $product->variant_value) {
                $product->variant_options_array = json_decode($product->variant_option);
    ...
    public function getProductFormData()
    {
        ...
        $units = Unit::whereNull('base_unit')->orderBy('unit_name')->get(['id', 'unit_code', 'unit_name']);
        $taxes = Tax::where('is_active', true)->orderBy('name')->get(['id', 'name', 'rate']);
    ...
    public function saveProduct(Request $request)
    {
        try {
            ...
            // Convert boolean-like strings to 0/1 (FormData sends 'true'/'false' as strings)
            $boolFields = ['featured', 'is_embeded', 'is_variant', 'is_diffPrice', 'is_batch', 'is_imei', 'promotion', 'is_active'];
            ...
            // Handle warranty and guarantee
            if (!isset($data['warranty']) || !$data['warranty']) {
                unset($data['warranty']);
                unset($data['warranty_type']);
            }
            if (!isset($data['guarantee']) || !$data['guarantee']) {
                unset($data['guarantee']);
                unset($data['guarantee_type']);
            }
            ...
            if ($data['type'] == 'combo') {
                ...
                $data['sale_unit_id'] = $data['sale_unit_id'] ?? 0;
            }
            ...
            // Handle promotional dates
            if (isset($data['starting_date']) && $data['starting_date']) {
                $data['starting_date'] = date('Y-m-d', strtotime($data['starting_date']));
            }
            ...
            if ($images && is_array($images)) {
                ...
                foreach ($images as $key => $image) {
                    ...
                    $this->diffSizeImageStore($img, $imageName);
            ...
            if (!isset($data['profit_margin']) || !$data['profit_margin']) {
                $data['profit_margin'] = 0;
            }

            // Default wastage_percent for non-combo products (required column, no DB default)
            if (!isset($data['wastage_percent']) || $data['wastage_percent'] === '') {
                $data['wastage_percent'] = '0';
            }

            // Default production_cost when not provided (required column, no DB default)
            if (!isset($data['production_cost']) || $data['production_cost'] === '') {
                $data['production_cost'] = 0;
            }

            // Default is_recipe when not provided (required column, no DB default)
            if (!isset($data['is_recipe']) || $data['is_recipe'] === '') {
                $data['is_recipe'] = 0;
            }

            // Handle alt_code (from Blade form)
            if (isset($request->alt_code)) {
            ...
            $variant_ids = [];
            if (isset($data['is_variant']) && isset($data['variant_name'])) {
            ...
            // Handle different warehouse pricing (update in place to preserve stock - matches Blade updateProduct logic)
            if (isset($data['is_diffPrice']) && isset($data['warehouse_id'])) {
                foreach ($data['diff_price'] as $key => $diff_price) {
                    if ($diff_price) {
                        $pw = ProductWarehouse::where('product_id', $product->id)
                            ->where('warehouse_id', $data['warehouse_id'][$key])
                            ->whereNull('variant_id')
                            ->first();
                        if ($pw) {
                            $pw->price = $diff_price;
                ...
                // When diffPrice unchecked: clear price on all warehouse records for this product (preserve qty)
