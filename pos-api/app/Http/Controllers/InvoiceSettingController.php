...
class InvoiceSettingController extends Controller
{
    ...
    /**
     * API: Get all invoice settings for Vue frontend.
     */
    public function getAllInvoiceSettings()
    {
        try {
            $settings = InvoiceSetting::orderBy('is_default', 'desc')
    ...
    /**
     * API: Save (create or update) invoice setting for Vue frontend.
     */
    public function saveInvoiceSetting(Request $request)
    {
        try {
            ...
            $data['status'] = isset($request->status) ? 1 : 0;
            $data['is_default'] = isset($request->is_default) ? 1 : 0;
            $data['size'] = $request->size ?? 'a4';
            $data['invoice_date_format'] = $request->invoice_date_format ?? 'd.m.y h:i A';
            $data['logo_height'] = $request->logo_height ?? 80;
            $data['logo_width'] = $request->logo_width ?? 120;
            $data['number_of_digit'] = $request->number_of_digit ?? 6;
            $data['header_text'] = $request->header_text;
            $data['footer_text'] = $request->footer_text;
            $data['primary_color'] = $request->primary_color;

            if ($request->id) {
                InvoiceSetting::findOrFail($request->id)->update($data);
            ...
            return response()->json([
                'status' => 500,
                'message' => 'Failed to save invoice setting',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    /**
     * API: Delete invoice setting.
     */
    public function deleteInvoiceSetting($id)
    {
        try {
            $invoice = InvoiceSetting::findOrFail($id);
            ...
            if ($invoice->is_default) {
                return response()->json([
                    'status' => 400,
                    'message' => 'Default invoice setting cannot be deleted. Set another as default first.'
                ], 400);
            }
            ...
            return response()->json([
                'status' => 500,
                'message' => 'Failed to delete invoice setting',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    /**
     * API: Set invoice setting as default.
     */
    public function setDefaultInvoiceSetting($id)
    {
        try {
            InvoiceSetting::where('is_default', 1)->update(['is_default' => 0]);
            InvoiceSetting::findOrFail($id)->update(['is_default' => 1]);

            return response()->json([
                'status' => 200,
                'message' => 'Default invoice setting updated successfully'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 500,
