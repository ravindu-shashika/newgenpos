...
class SaleController extends Controller
{
                    ...
                    <form action="'.route('sale.wappnotification').'" method="POST" style="display:inline;">
                      '.csrf_field().'
                        ...
                        <button type="submit" class="btn btn-link">
                            <i class="fa fa-whatsapp"></i> '.__('db.invoice_to_hatsapp').'
                        </button>
                    </form>
                </li>';
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')) {
            ...
            if(in_array('restaurant',explode(',',$general_setting->modules))){
                ...
                return view('backend.sale.create',compact('currency_list', 'all_permission', 'lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_reward_point_setting_data','options', 'numberOfInvoice', 'custom_fields', 'lims_customer_group_all', 'lims_table_list', 'service_list', 'waiter_list'));
    ...
    public function store(StoreSaleRequest $request)
    {
        ...
        /*try {*/
            ...
            if(isset($data['pos'])) {
                $clientRef = trim((string) ($data['reference_no'] ?? ''));
                if ($clientRef === '') {
                    $data['reference_no'] = $this->generateCompactInvoiceReference('posr-');
                } else {
                    $data['reference_no'] = $this->compactReferenceNo($clientRef);
                }
            ...
            else if($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && (is_array($data['paid_amount']) ? array_sum($data['paid_amount']) : (float) $data['paid_amount']) > 0)) {
                foreach($data['paid_by_id'] as $key=>$value)
                {
        ...
        if (request()->ajax() || $request->expectsJson() || $request->is('api/*')) {
            ...
            elseif ($data['pos']) {
                if ((int) $lims_sale_data->sale_status === 3) {
                    return response()->json(['id' => $lims_sale_data->id]);
                }
    ...
    /** POS / Flutter: prefix + datetime or seq, no dashes (e.g. kbt20260613202614). */
    private function generateCompactInvoiceReference(string $defaultPrefix = 'posr-'): string
    {
        $invoice_settings = InvoiceSetting::active_setting();
        $invoice_schema = InvoiceSchema::latest()->first();
        $show_active_status = json_decode($invoice_settings->show_column ?? '{}');
        $prefix = preg_replace('/[\s-]+/', '', (string) ($invoice_settings->prefix ?? $defaultPrefix));

        if (isset($show_active_status->active_generat_settings) && (int) $show_active_status->active_generat_settings === 1) {
            if ($invoice_settings->numbering_type === 'sequential') {
                if ($invoice_schema === null) {
                    InvoiceSchema::query()->create(['last_invoice_number' => $invoice_settings->start_number]);
            ...
            if ($invoice_settings->numbering_type === 'random') {
                return $prefix.rand(
                    (int) $invoice_settings->start_number,
                    (int) str_repeat('9', (int) $invoice_settings->number_of_digit)
                );
            }
        }
    ...
    private function compactReferenceNo(string $reference): string
    {
    ...
    {
        if (request()->boolean('app') || request()->query('app') === 'true') {
            $spaUrl = rtrim((string) env('SPA_URL', 'http://localhost:5173'), '/');
            $hashPath = $id ? "/pos/{$id}" : '/pos';

            return redirect("{$spaUrl}/#{$hashPath}");
        }
    ...
    public function getProducts($warehouse_id, $key, $cat_or_brand_id)
    {
        ...
        */
        ...
        $lims_product_list = $stockQuery
                            ...
                            ->orderBy('name', 'asc')
                            ->paginate(max(1, (int) (PosSetting::latest()->first()?->product_number ?: 15)));
