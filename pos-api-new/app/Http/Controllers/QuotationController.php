...
class QuotationController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($this->wantsSpaResponse($request)) {
            return app(QuotationDashboardController::class)->index($request);
        }
    ...
    /** Resource route fallback — old Blade used list modal + productQuotationData, not a show page. */
    public function show(Request $request, $id)
    {
        if ($this->wantsSpaResponse($request)) {
            return app(QuotationDashboardController::class)->show($request, $id);
        }
    ...
    {
        if ($this->wantsSpaResponse($request)) {
            return app(QuotationDashboardController::class)->createForm($request);
        }
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $embedFlag = explode('?', $product_data[3] ?? '0')[0] ?? '0';
        if ($embedFlag && $embedFlag !== '0') {
        ...
        if(!$lims_product_data) {
            ...
            if ($lims_product_data) {
                ...
                $lims_product_data->is_variant = true;
            }
        }
    ...
    {
        if ($this->wantsSpaResponse($request)) {
            return app(QuotationDashboardController::class)->editForm($request, $id);
        }
    ...
    public function genInvoice($id)
{
    try {
        ...
        $supportedIdentifiers = [
            'al', 'fr_BE', 'pt_BR', 'bg', 'cs', 'dk', 'nl', 'et', 'ka', 'de',
            'fr', 'hu', 'id', 'it', 'lt', 'lv', 'ms', 'fa', 'pl', 'ro', 'sk',
            'es', 'ru', 'sv', 'tr', 'tk', 'ua', 'yo'
        ];
