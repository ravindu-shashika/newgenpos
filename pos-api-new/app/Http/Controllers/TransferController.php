...
class TransferController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($this->wantsSpaResponse($request)) {
            return app(TransferDashboardController::class)->index($request);
        }
    ...
    {
        if ($this->wantsSpaResponse(request())) {
            return app(TransferDashboardController::class)->createForm(request());
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($this->wantsSpaResponse($request)) {
            return $this->spaJson($request, [
                ...
                'data' => [
                    'id' => $lims_transfer_data->id,
                    'reference_no' => $lims_transfer_data->reference_no,
                ],
            ], 201);
        }
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        if ($embedFlag && $embedFlag !== '0') {
            ...
            $qty = $product_data[4] ?? ($product_info[3] ?? 1);
        }
    ...
    {
        if ($this->wantsSpaResponse(request())) {
            return app(TransferDashboardController::class)->importForm(request());
        }
    ...
    public function importTransfer(Request $request)
    {
        ...
        while (!feof($file_handle) ) {
            ...
            if($current_line && $i > 0){
                ...
                if(!$product_data[$i-1]) {
                    $message = __('db.Product does not exist!');
                    if ($this->wantsSpaResponse($request)) {
                        return $this->spaJson($request, ['message' => $message], 422);
                    }
                    return redirect()->back()->with('message', $message);
                ...
                if(!$unit[$i-1]) {
                    $message = __('db.Purchase unit does not exist!');
                    if ($this->wantsSpaResponse($request)) {
                        return $this->spaJson($request, ['message' => $message], 422);
                    }
                    return redirect()->back()->with('message', $message);
                ...
                if(strtolower($current_line[4]) != "no tax"){
                    ...
                    if(!$tax[$i-1]) {
                        $message = __('db.Tax name does not exist!');
                        if ($this->wantsSpaResponse($request)) {
                            return $this->spaJson($request, ['message' => $message], 422);
                        }
                        return redirect()->back()->with('message', $message);
        ...
        $message = __('db.Transfer imported successfully');
        if ($this->wantsSpaResponse($request)) {
            return $this->spaJson($request, [
                'message' => $message,
                'data' => [
