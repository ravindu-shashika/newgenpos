...
class ProductController extends Controller
{
    ...
    protected function userHasProductPermission(array $names): bool
    {
        if (\App\Support\Permissions::bypassed()) {
            return true;
        }
        ...
        foreach ($names as $permission) {
            try {
                ...
                // permission name not registered in this tenant
            }
    ...
    protected function userCanProductIndex(): bool
    {
        return $this->userHasProductPermission([
            'products-index',
            'products.view',
            'products-index',
            'products',
    ...
    protected function userCanProductAdd(): bool
    {
        return $this->userHasProductPermission([
            'products-add',
            'products.create',
    ...
    protected function userCanProductEdit(): bool
    {
        return $this->userHasProductPermission([
            'products-edit',
            'products.edit',
    ...
    public function productData(Request $request)
    {
        ...
        foreach ($products as $key => $product) {
            ...
            // Legacy Blade DataTables action dropdown (React SPA builds actions client-side).
    ...
    /**
     * JSON payload for React product create / list filters (mirrors create() view data).
     */
    public function createInitialData(Request $request)
    {
        if (!$this->userCanProductAdd() && !$this->userCanProductEdit()) {
            return response()->json(['message' => __('db.Sorry! You are not allowed to access this module')], 403);
        }

        $general_setting = GeneralSetting::latest()->first();
        $modules = array_filter(explode(',', $general_setting->modules ?? ''));

        $comboCodes = [];
        foreach ($this->productWithoutVariant() as $product) {
            $comboCodes[] = htmlspecialchars($product->code) . ' (' . preg_replace('/[\n\r]/', '<br>', htmlspecialchars($product->name)) . ')';
        }
        foreach ($this->productWithVariant() as $product) {
            $comboCodes[] = htmlspecialchars($product->item_code) . ' (' . preg_replace('/[\n\r]/', '<br>', htmlspecialchars($product->name)) . ')';
        }

        $payload = [
            'brands' => Brand::where('is_active', true)->get(),
            'categories' => Category::where('is_active', true)->get(),
            'units' => Unit::where('is_active', true)->whereNull('base_unit')->get(),
            'all_units' => Unit::where('is_active', true)->get(),
            'taxes' => Tax::where('is_active', true)->get(),
            'warehouses' => Warehouse::where('is_active', true)->get(),
            'custom_fields' => CustomField::where('belongs_to', 'product')->get(),
            'modules' => array_values($modules),
            'margin_type' => $general_setting->margin_type ?? 2,
            'decimal' => $general_setting->decimal ?? 2,
            'number_of_product' => Product::where('is_active', true)->count(),
        ...
        if (in_array('restaurant', $modules, true)) {
            $payload['kitchens'] = DB::table('kitchens')->where('is_active', 1)->get();
            $payload['menu_type_list'] = DB::table('menu_type')->where('is_active', 1)->get();
        }
    ...
    /**
     * JSON payload for React product edit (mirrors edit() view data).
     */
    public function editInitialData(Request $request, $id)
    {
        ...
        $initialResponse = $this->createInitialData($request);
        if ($initialResponse->getStatusCode() !== 200) {
            return $initialResponse;
        }
        $initial = json_decode($initialResponse->getContent(), true);
        unset($initial['number_of_product']);

        $cost = (float) $product->cost;
        $price = (float) $product->price;
        $margin = (float) $product->profit_margin;
        if ($cost > 0 && $price > 0 && $product->profit_margin_type === 'percentage') {
            $calculatedMargin = (($price - $cost) / $cost) * 100;
            if (round($calculatedMargin, 2) != round($margin, 2)) {
                $product->profit_margin = round($calculatedMargin, 2);
            }
        }
        ...
        if ($product->type === 'combo' && $product->product_list) {
            ...
            $variantIds = explode(',', (string) $product->variant_list);
            $qtys = explode(',', (string) $product->qty_list);
            $prices = explode(',', (string) $product->price_list);
            $wastages = explode(',', (string) $product->wastage_percent);
            $comboUnits = explode(',', (string) $product->combo_unit_id);
            foreach ($ids as $key => $pid) {
                $pid = trim($pid);
                if ($pid === '') {
                    continue;
                }
                $comboProduct = Product::find($pid);
                if (!$comboProduct) {
                    continue;
                }
                $comboProducts[] = [
                    ...
                    'code' => $comboProduct->code,
                    'variant_id' => isset($variantIds[$key]) ? trim($variantIds[$key]) : '',
                    'qty' => isset($qtys[$key]) ? $qtys[$key] : 1,
                    'unit_price' => isset($prices[$key]) ? $prices[$key] : $comboProduct->price,
                    'wastage_percent' => isset($wastages[$key]) ? $wastages[$key] : 0,
                    'combo_unit_id' => isset($comboUnits[$key]) ? trim($comboUnits[$key]) : '',
                ];
            }
        }
        ...
        $relatedProducts = $relatedIds
            ? Product::whereIn('id', $relatedIds)->get(['id', 'name', 'code', 'image'])->values()
            : collect();
        $extras = $extraIds
            ? Product::whereIn('id', $extraIds)->get(['id', 'name', 'code', 'image'])->values()
            : collect();

        $previousImages = array_values(array_filter(explode(',', (string) $product->image)));

        $customFieldValues = [];
        foreach ($initial['custom_fields'] ?? [] as $field) {
            $fieldRow = is_array($field) ? $field : (array) $field;
            $fieldName = str_replace(' ', '_', strtolower($fieldRow['name']));
            $customFieldValues[$fieldName] = $product->{$fieldName} ?? null;
        }

        $initialStock = [];
        $hasInitialStock = false;
        if (!$product->is_variant && !$product->is_batch && !$product->is_imei) {
            $initialStock = $this->initialStockByWarehouse($product->id);
            $hasInitialStock = !empty($initialStock);
        }

        return $this->spaJson($request, array_merge($initial, [
            'product' => $product,
            'variants' => $variants,
    ...
    /**
     * Ecommerce related products / restaurant extras search (Blade: GET search/{term}).
     */
    public function catalogSearch(Request $request, string $term)
    {
        try {
            if (strlen($term) < 3) {
            ...
            $columns = ['id', 'name', 'code', 'cost', 'tax_id'];
            if (\Illuminate\Support\Facades\Schema::hasColumn('products', 'image')) {
                $columns[] = 'image';
            }

            $products = $query->select($columns)->limit(20)->get();
            $taxRates = Tax::whereIn('id', $products->pluck('tax_id')->filter()->unique())->pluck('rate', 'id');

            $payload = $products->map(function ($p) use ($taxRates) {
                $row = [
                    'id' => $p->id,
                    'name' => $p->name,
                    'code' => $p->code,
    ...
    public function store(Request $request)
    {
        ...
        if (in_array('ecommerce', $modules, true)) {
            ...
            $data['related_products'] = rtrim((string) $request->input('products', ''), ',');

            $data['in_stock'] = $request->has('in_stock') ? $request->input('in_stock') : 0;
            $data['is_online'] = $request->has('is_online') ? $request->input('is_online') : 0;
        }
        ...
        if (in_array('restaurant', $modules, true)) {
            ...
            $data['slug'] = str_replace('\/', '/', $data['slug']);
            $data['related_products'] = rtrim((string) $request->input('products', ''), ',');
            $data['extras'] = rtrim((string) $request->input('extras', ''), ',');

            $data['is_online'] = $request->has('is_online') ? $request->input('is_online') : 0;
            $data['is_addon'] = $request->has('is_addon') ? $request->input('is_addon') : 0;
            $data['kitchen_id'] = $request->input('kitchen_id');

            if ($request->menu_type) {
                $data['menu_type'] = is_array($request->menu_type)
                    ? implode(',', $request->menu_type)
                    : $request->menu_type;
            }
        }
    ...
    public function autoPurchase($product_data, $warehouse_id, $stock)
    {
        ...
        $data['purchase_type'] = 'initial stock';
    ...
    /**
     * Opening quantities recorded when the product was created with initial stock.
     */
    private function initialStockByWarehouse(int $productId): array
    {
        $rows = DB::table('product_purchases')
            ->join('purchases', 'purchases.id', '=', 'product_purchases.purchase_id')
            ->where('product_purchases.product_id', $productId)
            ->where(function ($query) {
                $query->where('purchases.purchase_type', 'initial stock')
                    ->orWhere(function ($query) {
                        $query->whereNull('purchases.supplier_id')
                            ->whereNull('purchases.purchase_type')
                            ->where('purchases.item', 1);
                    });
            })
            ->select('purchases.warehouse_id', 'product_purchases.qty')
            ->get();

        $initialStock = [];
        foreach ($rows as $row) {
            $initialStock[(int) $row->warehouse_id] = (float) $row->qty;
        }
    ...
    public function history(Request $request)
    {
        ...
        $warehouse_id = $request->input('warehouse_id') ? (int) $request->input('warehouse_id') : 0;
        ...
        $product_data = $product_id ? Product::select('name', 'code')->find($product_id) : null;
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        ...
        if ($this->wantsSpaResponse($request)) {
            return $this->spaJson($request, [
                ...
                'show_warehouse_filter' => Auth::user()->role_id <= 2,
            ]);
        }
    ...
    public function updateProduct(Request $request)
    {
        ...
        try {
            ...
            if ($request->ajax() || $this->wantsSpaResponse($request)) {
                return $this->spaJson($request, [
                    'message' => __('db.Failed to update product Please try again'),
                    'error' => config('app.debug') ? $e->getMessage() : null,
                ], 422);
            }
    ...
    public function generateCode(Request $request)
    {
        ...
        if ($this->wantsSpaResponse($request)) {
            return $this->spaJson($request, ['code' => $id]);
        }
        return $id;
    }
    ...
    public function search(Request $request)
    {
        ...
        if (!$lims_product_data) {
            if ($this->wantsSpaResponse($request)) {
                return $this->spaJson($request, [], 404);
            }
            return [];
        }
        ...
        $product = [
            ...
            $lims_product_data->qty,
            $lims_product_data->price,
            $lims_product_data->id,
        ];

        if ($this->wantsSpaResponse($request)) {
            return $this->spaJson($request, $product);
        }

        return $product;
    }

    public function saleUnit(Request $request, $id)
    {
        $unit = Unit::where([
            'base_unit' => $id,
            'is_active' => true,
        ...
        if ($this->wantsSpaResponse($request)) {
            return $this->spaJson($request, ['data' => $unit]);
        }
    ...
    public function printBarcode(Request $request)
    {
        ...
        if ($request->input('data')) {
            $preLoadedproducts = $this->resolveBarcodePreloadProducts($request);
        }

        $barcodeRows = Barcode::orderBy('id')->get();
        $default = $barcodeRows->where('is_default', 1)->first();

        if ($this->wantsSpaResponse($request)) {
            $barcodeSettings = $barcodeRows->map(function ($row) {
                return [
                    'id' => $row->id,
                    'name' => $row->name,
                    ...
                    'paper_width' => (float) $row->paper_width,
                    'paper_height' => (float) $row->paper_height,
                    'top_margin' => (float) $row->top_margin,
                    'left_margin' => (float) $row->left_margin,
                    'row_distance' => (float) $row->row_distance,
                    'col_distance' => (float) $row->col_distance,
                    'stickers_in_one_row' => (int) $row->stickers_in_one_row,
                    'stickers_in_one_sheet' => $row->stickers_in_one_sheet !== null
                        ? (int) $row->stickers_in_one_sheet
                        : null,
            ...
            return $this->spaJson($request, [
                ...
                'default_barcode_setting_id' => $default?->id,
                'warehouses' => $warehouses,
                'pre_loaded_products' => $preLoadedproducts,
                'product_catalog' => $this->printBarcodeProductCatalog(),
            ]);
        }
    ...
    private function printBarcodeProductCatalog(): array
    {
        $catalog = [];
        ...
        foreach ($this->productWithVariant() as $product) {
            $catalog[] = $product->item_code . ' (' . $product->name . ')';
        }
    ...
    protected function resolveBarcodePreloadProducts(Request $request): array
    {
        $searchRequest = $request->duplicate(
            array_merge($request->query(), ['barcode' => '1'])
        );

        $result = $this->limsProductSearch($searchRequest);
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        if ($this->wantsSpaResponse($request)) {
            return $this->spaJson($request, $products);
        }
