...
class ExpenseController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($this->wantsSpaResponse($request)) {
            return app(ExpenseDashboardController::class)->index($request);
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($role->hasPermissionTo('expenses-edit')) {
            ...
            if (!$lims_expense_data) {
                if ($this->wantsSpaResponse($request)) {
                    return $this->spaJson($request, ['message' => __('db.Expense not found')], 404);
                }
                return redirect()->back()->with('not_permitted', __('db.Expense not found'));
            }
            $lims_expense_data->date = date(config('date_format') ?: 'd-m-Y', strtotime($lims_expense_data->created_at->toDateString()));
            if ($this->wantsSpaResponse($request)) {
                return $this->spaJson($request, ['expense' => $lims_expense_data]);
            }
    ...
    {
        if ($this->wantsSpaResponse($request)) {
            return app(ExpenseDashboardController::class)->destroy($request, $id);
        }
